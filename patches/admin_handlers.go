...
func (h *AdminHandlers) registerAPIRoutes(api *mux.Router, enforceWrite bool) {
	...
	filesApi.Handle("/upload-init", wrapWrite(h.fileBrowserHandlers.InitChunkedUpload)).Methods(http.MethodPost)
	filesApi.Handle("/upload-chunk", wrapWrite(h.fileBrowserHandlers.UploadChunk)).Methods(http.MethodPost)
	filesApi.Handle("/upload-complete", wrapWrite(h.fileBrowserHandlers.CompleteChunkedUpload)).Methods(http.MethodPost)
	filesApi.Handle("/upload-abort", wrapWrite(h.fileBrowserHandlers.AbortChunkedUpload)).Methods(http.MethodPost)
